...
import ir.darshub.app.data.DmMessage
import ir.darshub.app.data.DmPartner
...
class DmThreadViewModel : ViewModel() {
    ...
    private fun sendMediaUri(ctx: Context, type: String, uri: Uri, duration: Int?, onErr: (String) -> Unit) {
        viewModelScope.launch {
            ...
            val err = ir.darshub.app.ui.messages.ChatMedia.sendMedia(ctx, "dm", target(), type, uri, duration) { pct ->
    ...
    fun sendVoice(file: File, durationSec: Int, onErr: (String) -> Unit) {
        viewModelScope.launch {
            state.update { it.copy(uploadPercent = 0) }
            val err = ir.darshub.app.ui.messages.ChatMedia.sendMedia(App_context(), "dm", target(), "voice", Uri.fromFile(file), durationSec) { pct ->
                state.update { it.copy(uploadPercent = pct) }
            }
            state.update { it.copy(uploadPercent = null) }
    ...
    private fun App_context(): Context = ir.darshub.app.App.instance

    fun sendMusic(musicId: Int, onErr: (String) -> Unit) {
        viewModelScope.launch {
            val err = ir.darshub.app.ui.messages.ChatMedia.sendRef("dm", target(), "music", musicId = musicId)
    ...
    fun sendSticker(stickerId: Int, onErr: (String) -> Unit) {
        viewModelScope.launch {
            val err = ir.darshub.app.ui.messages.ChatMedia.sendRef("dm", target(), "sticker", stickerId = stickerId)
            if (err != null) onErr(err); poll()
        }
    }
